...
export async function editUserData(req, res) {
  try {
    const { nickname, dob, note } = req.body;
    const updatedData = await userDataSchema.updateOne(
      { userId: req.user.UserID },
      { $set: { nickname, dob, note } }
    );
    res
      .status(200)
      .send({ message: "Data updated successfully!", data: updatedData });
  } catch (error) {
    console.error(error);
    res
      .status(500)
      .send({ message: "Failed to update data. Please try again." });
  }
}
