...
const Profile = () => {
  ...
  const getPosts = async () => {
    try {
      const res = await axios.get("http://localhost:3006/api/getPosts", {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (res.status === 200) {
  ...
  const handleClick = async (e) => {
    ...
    if (!token) {
      ...
      try {
        const res = await axios.delete("http://localhost:3006/api/deleteuser", {
          headers: { Authorization: `Bearer ${token}` },
        });
        if (res.status == 200) {
          alert(res.data.msg);
    ...
    <div className="container">
      <div className="left-side">
        ...
        {userData ? (
          ...
          <>
            <div>Bio: Add Bio? need to create !</div>
            <Link to={"/addData"}>
